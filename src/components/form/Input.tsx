...
export type InputProps = {
  ...
  // Props opsional untuk penggunaan di luar FormProvider
...
export default function Input({
  ...
  // Aman dari error meskipun tidak dalam FormProvider
  let finalRegister = externalRegister;
  let error = externalError;

  try {
    const form = useFormContext();
    finalRegister = externalRegister ?? form.register;
    error = externalError ?? get(form.formState.errors, id);
  } catch (e) {
    // Not in form context, fallback to basic input
  }
  ...
  return (
    <div className="w-full flex flex-col gap-2">
      ...
      <div className="relative flex w-full gap-0">
        <div className="pointer-events-none absolute h-full w-full rounded-md border-gray-300 ring-1 ring-inset ring-gray-500" />
        ...
        >
          {LeftIcon && (
            <div
              className={clsxm(
                "absolute left-0 top-0 h-full flex items-center justify-center pl-2.5 text-lg text-gray-900 md:text-xl",
                leftIconClassName,
              )}
            >
              <LeftIcon />
          ...
          <input
            {...(finalRegister ? finalRegister(id, validation) : {})}
          ...
          {RightIcon && type !== "password" && (
            <div
              className={clsxm(
                "absolute bottom-0 right-0 h-full flex items-center justify-center pr-2.5 text-lg text-gray-900 md:text-xl",
                rightIconClassName,
              )}
            >
              <RightIcon />
          ...
          {type === "password" && (
            <div
              className={clsxm(
                "absolute bottom-0 right-0 h-full flex items-center justify-center pr-3 text-lg text-gray-900 md:text-xl cursor-pointer",
                rightIconClassName,
              )}
      ...
      {!hideError && error && (
        <ErrorMessage className={errorMessageClassName}>
          {typeof error === "string" ? error : "Input tidak valid"}
        </ErrorMessage>
      )}
      {helperText && (
